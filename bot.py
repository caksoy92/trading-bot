from flask ifrom flask import Flask, request, jsonify
...
def db_kur():
    ...
    c.execute("""CREATE TABLE IF NOT EXISTS pozisyonlar (
        symbol TEXT PRIMARY KEY, veri TEXT)""")
    c.execute("""CREATE TABLE IF NOT EXISTS islem_gecmisi (
        id SERIAL PRIMARY KEY, veri TEXT)""")
...
def pozisyon_yaz(symbol, veri):
    conn = db_baglan()
    c = conn.cursor()
    c.execute("""INSERT INTO pozisyonlar (symbol, veri) VALUES (%s, %s)
        ON CONFLICT (symbol) DO UPDATE SET veri=%s""",
        (symbol, json.dumps(veri), json.dumps(veri)))
    conn.commit()
    conn.close()
...
if __name__ == '__main__':
    ...
    app.run(host='0.0.0.0', port=8080)mport Flask, request, jsonify
from datetime import datetime
import requests
import os
import threading
